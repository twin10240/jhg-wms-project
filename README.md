...
> 🔗 **라이브 데모: [wms.jhgsoft.com](https://wms.jhgsoft.com)** — `demo` / `demo1234` (열람용 `OPERATOR` 계정)
> nginx + 인스턴스 3개 구성을 그대로 공개합니다 — 응답 헤더 `X-Served-By`가 요청마다 바뀝니다.
> 개발 머신에서 구동해 상시 가동은 아니며, 화면의 AI 분류·브리핑은 시연 데이터를 넣을 때 만든 결과입니다
> (공개 주소에서 새 AI 호출은 꺼져 있습니다 → [공개 스택](#공개-스택-docker-compose)).
...
**발주 근거 — 원장에서 뽑은 숫자 (V10.0)**

![발주 근거 패널](docs/images/purchase-order-advice.png)

발주 폼 **위**에 "무엇을 얼마나 발주할지"를 판단할 실측치를 띄웁니다. 소진이 임박한 순으로 정렬하며,
**표본 일수를 행마다 밝힙니다** — 원장이 14일치인데 30으로 나누면 일평균이 절반이 되고 소진 예상일이
두 배로 늘어, 재고가 버틸 거라고 잘못 말하게 됩니다. **여기에 AI는 없습니다.** 원장 SHIP 델타의
합이라 추론이 아니라 산술이고, 그래서 "참고" 태그를 붙이지 않습니다. 권장 발주량도 내지 않습니다 —
목표 재고일수는 WMS가 갖고 있지 않은 값이라 사람이 정합니다.
...
### 발주 근거 패널과 메모 자동 분류 (V10.0)

발주 화면(`/admin/purchase-orders`)에 **서버사이드 렌더 한 번**으로 붙는 표입니다.
새 엔드포인트도 JS도 없습니다.

| 열 | 어떻게 나오나 |
|---|---|
| 최근 출고 (표본 일수) | 창 30일 안의 원장 `SHIP` 델타 합. 부호를 뒤집어 나간 수량으로 냅니다 |
| 일평균 | 합계 ÷ **표본 일수**. 분모는 창 길이가 아니라 **그 상품 원장이 실제로 존재한 일수**입니다 |
| 가용 | `onHand - reserved` |
| 소진 예상 | 가용 ÷ 일평균. **일평균이 0이면 `—`이지 `0일`이 아닙니다** — 잴 것이 없는 것과 오늘 소진되는 것은 다릅니다 |
| 직전 발주 | 그 상품의 가장 최근 발주(번호·발주일·수량·상태) |

- 정렬은 소진 예상 오름차순, **소진 예상이 없는 상품은 맨 뒤**입니다. 위로 올리면 안 붙는 순서가 됩니다.
- **직전 발주는 날짜가 아니라 `createdAt` 시각으로 고릅니다.** 일 단위로 비교하면 같은 날 발주 둘이
  동률이 되고, 그때 승자가 목록 화면의 정렬(열린 것 먼저·종료된 것 뒤)에 좌우됩니다. 그러면 같은 날
  입고 완료된 발주가 **아직 안 들어온 발주를 덮어** 패널이 "직전 발주 입고 완료"라 말하고 담당자가
  재발주합니다. 당일 발주·당일 입고는 이 시스템 데이터의 실제 모습입니다.
- 재고 행과 발주 전건은 **화면 핸들러가 이미 조회한 것을 넘겨받습니다**. 서비스가 다시 부르면
  컨트롤러는 트랜잭션이 아니라 1차 캐시에 안 걸려 같은 쿼리가 진짜로 두 번 나갑니다.

**발주 메모 자동 분류**는 [반품 사유 자동 분류](#반품-사유-자동-분류-v40)와 같은 틀입니다 —
커밋 후 별도 스레드, 구조화 출력, **참고 표시 전용**(발주 내용에 영향 없음), 키 없으면 분류만 비활성.

![발주 메모 분류](docs/images/purchase-order-memo-classification.png)
...
분류 둘과 달리 **동기**이고 **구조화 출력이 없습니다** — 문단이라 가둘 스키마가 없습니다.
참고 표시 전용이며 발주 내용에 영향을 주지 않습니다.
품질은 13건(정상 8 + 네거티브 5) 평가셋으로 5회차까지 측정했습니다 — 숫자 대조는 기계가, 유용성은
LLM-as-judge가 채점하고, 사람이 망가뜨린 네거티브가 judge 자체를 검증합니다.
[측정 결과](docs/wms-briefing-eval.md).
...
| `400` | 검증 실패 — 미출고 주문, 출고 내역에 없는 상품, 누적 반품량 초과, 수량 0 이하, `orderRequestKey` 누락·형식 오류·`orderId` 불일치 |
...
> `requestKey`와 다른 값이니 헷갈리면 안 됩니다. **필수 항목**이고, WMS는 이 값으로 예약을
> **단건으로** 찾습니다. 없으면 `400`입니다.
> 예전에는 선택 항목이라, 없으면 `orderId`로 가장 최근 예약을 고르는 레거시 경로를 탔습니다.
> `orderId`는 유일하지 않아서(OMS DB 초기화로 재사용) 그 추측은 옛 주문의 반품을 새 주문에
> 붙이거나 실제로 출고된 상품을 "출고 내역에 없다"고 거절했습니다 — 예약·출고·해제·송장조회가
> 2026-09-03에 `requestKey`로 옮겨간 것과 같은 이유입니다. OMS가 `e6e6a35`부터 모든 반품에
> 이 값을 실어 2026-09-14에 레거시 경로를 지웠습니다(요청서: `docs/oms-request-rma-order-request-key.md`).
> `orderId`와 같이 받아 서로 맞는지 검사하고, 어긋나면 `400`으로 막습니다.
> 누적 반품량 검증은 아직 `orderId` 기준입니다 — 틀려도 과다 집계로 **거절하는** 방향이라 남겨 뒀습니다.
...
품질은 41건 평가셋으로 6회차까지 측정했습니다(최신 40/41) — [측정 결과](docs/wms-classification-eval.md).
...
실사 보고서는 별도 Skill입니다 —
[`.claude/skills/wms-cycle-count-report/SKILL.md`](.claude/skills/wms-cycle-count-report/SKILL.md).
판단 기준이 다릅니다: 코호트·미분류 대신 **반려 세션 제외**(정확도 분모에서 빠진 항목 수를
반드시 밝힌다), `netQty` 상쇄, 그리고 **없는 기준을 만들지 않기**(WMS는 목표 정확도도
업계 벤치마크도 갖고 있지 않다)가 들어갑니다.
...
> **보고서를 볼 때 조회 구간에 주의하세요.** 이 개발 DB에는 시연용 데이터(`DEMO-` 30건)와
> 수동 통합검증 잔재(19건)가 섞여 있어, 구간을 잘못 잡으면 잔재만 측정합니다
> (실측: 2026-08 구간은 미분류 76%, 2026-09-01 구간은 0%).
> 어느 구간을 물어야 하는지는 [`mcp-server/README.md`](mcp-server/README.md)의 "개발 DB의 유효 구간" 절에 있습니다.
...
반품도 `POST /api/returns`의 필수 항목 `orderRequestKey`로 예약을 단건으로 찾습니다
(2026-09-14부터, [요청서](docs/oms-request-rma-order-request-key.md)).
...
> **prod(Railway)는 2026-09-09 현재 미적용입니다.** 재배포할 때 **이 SQL을 새 코드보다 먼저**
> 돌리세요 — 순서가 뒤집히면 옛 `order_id` UNIQUE가 살아남아 재사용된 주문 번호를 거부합니다.
