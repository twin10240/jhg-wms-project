...
@Slf4j
public class ClaudePurchaseOrderMemoClassifier implements PurchaseOrderMemoClassifier {

    private final AnthropicClient client;
    private final PurchaseOrderMemoJsonParser parser;
    private final String model;
    private final long maxTokens;
    private final String systemPrompt;
    private final JsonOutputFormat.Schema schema;

    public ClaudePurchaseOrderMemoClassifier(AnthropicClient client, ObjectMapper objectMapper,
                                             String model, long maxTokens) {
        this.client = client;
        this.parser = new PurchaseOrderMemoJsonParser(objectMapper);
        this.model = model;
        this.maxTokens = maxTokens;
        // 프롬프트·스키마는 기동 시 한 번 읽는다. 파일이 없으면 그건 배포 사고라 기동에서 드러나야 한다.
        this.systemPrompt = readResource("prompts/purchase-order-memo-classification.txt");
        this.schema = toSchema(objectMapper, readResource("prompts/purchase-order-memo-classification-schema.json"));
    }

    @Override
    public Optional<Classification> classify(String memo) {
        if (memo == null || memo.isBlank()) return Optional.empty();
        ...
        try {
            ...
            log.warn("발주 메모 분류 호출 실패(무시): {}", e.toString());
