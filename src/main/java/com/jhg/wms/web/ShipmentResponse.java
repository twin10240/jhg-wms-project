 ...
 * 주문이 다른 번호를 받는다. 상관관계는 `orderId`로 잡는다(`ShipResponse`와 같은 규칙).
 */
public record ShipmentResponse(Long orderId, String carrierCode, String carrierName,
                               String trackingNumber, Instant issuedAt, Instant deliveredAt) {

    public static ShipmentResponse from(Reservation r) {
        return new ShipmentResponse(r.getOrderId(), r.getCarrierCode(), Reservation.CARRIER_NAME,
