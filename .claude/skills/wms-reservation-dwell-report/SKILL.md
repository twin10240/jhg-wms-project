...
2. **`stillOpen`** — 구간 끝(`to`) 시점의 미종료 예약 재고량이다. **`from`에 의존하지 않는다** —
   그 시각에 아직 안 끝난 예약을 전부 세므로, 1일짜리 구간과 30일짜리 구간이 같은 값을 낼 수 있다.
   그래서 표본 크기와 직접 비교할 수치가 아니다(하나는 순간의 잔량, 하나는 구간 동안의 흐름이다) —
   좁은 구간일수록 `stillOpen`이 표본보다 커지는 것이 정상이다. 다만 여전히 **생존 편향의 크기**이므로
   밝히기는 밝힌다: 오래 붙들려 있는 예약일수록 아직 안 끝나 분포에 안 잡히고, 이 수를 밝히지 않고
   중앙값을 인용하면 실제보다 짧게 말하는 것이다.
...
| 유난히 긴 체류 하나를 그날 실제로 오래 걸린 사례로 인용한다 | 송장 없는 옛 출고 건이 재시도로 issuedAt=지금이 됐을 수 있다 — 출고 시각이 아니라 메운 시각이라 그 구간의 max·p90을 부풀린다 |
