...
요청에 기간이 없으면 **임의로 고르지 말고 사용자에게 묻는다.**

"지난달"·"최근 30일"·"최근 완결 월"은 그럴듯한 기본값이지만 **데이터가 그 구간에 있다는
보장이 없다.** 도구 넷 중 어느 것도 "어느 구간에 데이터가 있는지"를 알려주지 않으므로
추론으로는 알 수 없다. 빈 구간이나 검증용 잔재만 담긴 구간을 고르면 보고서 전체가
잘못된 바닥 위에 선다 — 숫자를 아무리 정확히 읽어도 소용없다.

실측(기간 없이 5회 시행): 다섯 번 모두 되묻지 않고 **서로 다른 네 구간**을 골랐다
(지난달 / 최근 30일 / 최근 완결 월 / 최근 3개월). 유효 데이터 구간을 잡은 것은 하나도
없었고 한 번은 **빈 결과**였다. 추론이 나빠서가 아니라 알 수 없는 것을 추측했기 때문이다.
