...
**먼저 조회해서 데이터 위치를 확인했더라도 묻는다.** 넓은 구간을 훑어 데이터가 어디 있는지
알아냈다 해도, 그 구간이 사용자가 보려던 구간인지는 도구가 답해주지 않는다. 확인한 것은
데이터가 어디 있는지지 사용자가 무엇을 보려는지가 아니다. 훑는 것은 되묻기를 **대신하는**
절차가 아니라, 되물을 때 선택지를 같이 제시하기 위한 준비다.
