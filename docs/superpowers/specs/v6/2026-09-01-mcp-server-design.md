...
**결정적인 변화는 LLM이 클라이언트로 옮겨간다는 것이다.** 이 분석 경로에는 WMS 쪽 LLM 호출이
없다(반품 사유 자동 분류(V4.0)는 이미 WMS 안에서 Claude API를 부른다 — 이 기능과는 별개 경로다).
...
>
> **더 날카로운 결과는 반대 방향이다.** MCP 서버 프로세스도 같은 `wms.basic` 자격증명을 들고
> 있어야 하는데, `apiChain`은 `/api/**` 전체에 `anyRequest()`를 허용한다 — 쓰기 엔드포인트
> (`POST /api/returns`, `POST /api/return-status-events`, 예약·출고·해제)까지 포함한다. 즉
> "읽기 전용"은 HTTP 계층이 강제하는 것이 아니라 MCP 서버가 위 네 URL만 부른다는 **애플리케이션
> 코드의 약속**일 뿐이다. 이것이 실제로 의미하는 것: MCP 서버의 env가 새면 쓰기 가능한 자격증명이
> 새는 것이고, 접근 로그는 MCP의 읽기와 OMS의 쓰기를 구분하지 못하며, MCP 접근을 회수하려면
> OMS의 자격증명까지 함께 회전해야 한다. 상한은 그대로다 — 롤을 하나 더 두는 것.
...
- **범주 표의 건수와 상세 목록의 행 수는 다를 수 있다 — 오류가 아니다.** 범주 분해는 *반품*을 세고
  상세 목록은 *품목*을 편다. 반품 하나가 상품 둘을 담으면 행이 둘이다. 실측(2026-09-02):
  미분류 13건인데 미분류 상세는 14행이었다. 이 차이를 불일치로 보고하지 않는다.
...
- **기간에 상한이 없다.** `productReturnRates`는 구간 내 모든 SHIP 트랜잭션을, `allDetails`는
  구간 내 모든 반품 품목을 거르기 전에 메모리에 올린다(둘 다 화면 한 장 기준의 `ponytail:` 주석이
  달려 있다). 새 호출자는 모델이고 `from=2020-01-01` 같은 요청을 그럴듯하게 던질 수 있으며,
  상세 응답은 사유 원문을 그대로 토큰 예산에 태운다. 화면도 임의 구간을 받으므로 회귀는 아니지만,
  기간·응답 크기를 자르는 지점은 6-b(MCP 서버)가 맞다.
